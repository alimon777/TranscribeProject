...
import {
  UploadCloud, Brain, FileEdit,
  FolderKanban, Clock, Settings, User, Eye,
  History, AlertTriangle, CheckCircle, XCircle,
  ListFilter, PlusCircle, RefreshCw,
  InfoIcon,
  Loader2,
  Save,
  Download,
                  ...
                  <div className="grid gap-1.5 leading-none">
                    <label htmlFor="integrateKB" className="text-sm font-medium peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                      ...
                      <TooltipProvider delayDuration={100}>
                        <Tooltip>
                          <TooltipTrigger asChild><InfoIcon className="inline h-3.5 w-3.5 ml-1 text-muted-foreground relative -top-px" /></TooltipTrigger>
...
// --- Transcription Repository Page (NEW with TreeView) ---
const RepositoryPage = () => {
  ...
  // load both folder tree + transcriptions on mount
  ...
  // Tree handlers
  const handleAdd = useCallback(async (parentId, name, proposedId, path) => {
    const res = await treeApi.addFolder(parentId, name, proposedId);
    if (res.success) {
      setTreeData(t => addNodeRecursive(t, parentId, { id: res.finalId, name, children: [] }));
      return { success: true, finalId: res.finalId };
    }
    return { success: false };
  }, []);
  const handleRename = useCallback(async (id, newName, oldName) => {
  ...
  // filter transcriptions by folder + search
  const list = transcriptions
    .filter(t => !selectedFolder || t.folder_id === selectedFolder)
    .filter(t => !search || t.session_title.toLowerCase().includes(search.toLowerCase()));

  return (
    <div className="p-4 md:p-6 w-full">
      <div className="mb-6 text-left pl-3 pb-2">
        <h1 className="text-2xl sm:text-3xl font-semibold mb-1 text-foreground">Transcription Repository</h1>
        <p className="text-md text-muted-foreground">Browse, organize, and search all stored transcriptions.</p>
      </div>
      <div className="flex flex-col md:flex-row gap-6 items-start">
        <aside className="w-64 border rounded p-2 space-y-2">
        ...
        <section className="flex-1 space-y-4">
          ...
          />
          <Card>
            <CardHeader><CardTitle>Transcriptions</CardTitle></CardHeader>
            <CardContent className="p-0">
              {list.length === 0 ? (
                    ...
                    {list.map(t => (
                      <TableRow key={t.id}>
                        <TableCell>
                          ...
                          <div className="text-xs text-muted-foreground">{t.source_file_name}</div>
                        </TableCell>
                        <TableCell>{new Date(t.processed_at).toLocaleDateString()}</TableCell>
                        <TableCell>{t.session_purpose}</TableCell>
...
export function IntegrationFolderDialog({
  isOpen,
  onClose,
  suggestedPath = [],      // e.g. ["Projects","CRM"]
  onConfirm                // fn(folderId, folderPathArray)
}) {
  const [treeData, setTreeData] = useState([]);
  const [selectedId, setSelectedId] = useState(null);
  const [isAddingUnder, setIsAddingUnder] = useState(null);
  ...
  useEffect(() => {
    if (isOpen) {
      setTreeData(getFolderTreeData());
      setSelectedId(null);
      setIsAddingUnder(null);
      setNewName('');
    }
  }, [isOpen]);

  const handleAdd = useCallback(async (parentId, name, propId, path) => {
    const res = await treeApi.addFolder(parentId, name, propId);
    if (res.success) {
      setTreeData(t =>
        addNodeRecursive(t, parentId, { id: res.finalId, name, children: [] })
      );
      // auto-select the newly created folder
      setSelectedId(res.finalId);
      return { success: true, finalId: res.finalId };
    }
    return { success: false };
  }, []);
  ...
  const handleConfirm = () => {
    // compute path array by walking up treeData
    ...
    const pathArray = selectedId ? buildPath(treeData, selectedId) : [];
    onConfirm(selectedId, pathArray);
    onClose();
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Select Integration Folder</DialogTitle>
          <DialogDescription>
            Suggested path: {suggestedPath.join(' / ') || '—'}<br />
