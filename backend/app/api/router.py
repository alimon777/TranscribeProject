...
def build_folder_tree(db: Session, parent_id: Optional[str] = None) -> List[FolderTreeNodeSchema]:
    # Fetches actual folders. Top-level folders will have parent_id=None.
    ...
    try:
        ...
        for folder_obj in folders_to_update:
            ...
            if folder_obj.count != actual_direct_count:
                folder_obj.count = actual_direct_count
                changed = True
        if changed:
            db.commit()
    finally:
...
def get_pending_transcriptions_for_review(db: Session = Depends(get_db)):
    ...
    query = db.query(Transcription).filter(Transcription.status.in_(pending_statuses)).order_by(Transcription.updated_at.desc())
...
def rename_folder(folder_id: str, folder_update: FolderUpdateSchema, db: Session = Depends(get_db)):
    ...
    # No "all" folder to worry about renaming in this virtual model
...
async def upload_and_process_transcription(metadata: TranscriptionProcessMetadataSchema = Body(...), db: Session = Depends(get_db)):
    ...
    db_transcription = Transcription(
        session_title=metadata.session_title or "Untitled Session", status=TranscriptionStatusEnum.PROCESSING,
        source_file_name="mock_file.mp4", processing_time_seconds=processing_time_sec,
        topic_names_csv=topic_names_csv_str, uploaded_at=datetime.utcnow()
    )
    ...
    return ProcessedTranscriptionReviewDataSchema(
        transcription_id=db_transcription.id,
        session_info={"title": db_transcription.session_title, "purpose": metadata.session_purpose, "domain": topic_names_csv_str or "N/A", "processingTime": f"{processing_time_sec // 60}m {processing_time_sec % 60}s"},
        cleaned_transcription=db_transcription.cleaned_transcript_text, generated_quiz=generated_quiz_text,
        knowledge_base_integration={"suggestionText": f"Related to '{metadata.primary_topic_names[0] if metadata.primary_topic_names else 'General'}'.", "proposedLocation": (metadata.primary_topic_names[:1] if metadata.primary_topic_names else ["General"]) + [db_transcription.session_title]}
    )
...
def save_transcription_as_draft(transcription_id: int, data: TranscriptionUpdateSchema, db: Session = Depends(get_db)):
    ...
    if data.folder_id is not None:
        ...
        if not target_folder: raise HTTPException(status_code=404, detail=f"Draft folder {data.folder_id} not found.")
        db_transcription.folder_id = data.folder_id
    elif data.folder_id is None and db_transcription.folder_id is not None:
        db_transcription.folder_id = None
...
def finalize_transcription_integration(transcription_id: int, target_folder_id: str = Body(..., embed=True), updated_data: Optional[TranscriptionUpdateSchema] = Body(None), db: Session = Depends(get_db)):
    ...
    if uuid.uuid4().int % 10 < 3:
        ...
        db.add(conflict)
        message = f"Integrated to '{target_folder.name}'. Potential conflict flagged."
    db.commit()
...
def get_repository_data(folder_id: Optional[str] = Query(None), search: Optional[str] = Query(None), sort_by: Optional[str] = Query("integrated_at"), sort_order: Optional[str] = Query("desc"), purpose_filters: Optional[str] = Query(None), db: Session = Depends(get_db)):
    ...
    # If folder_id is "all" or None, don't filter by folder_id
    ...
    if search:
        ...
        trans_query = trans_query.filter(
            (func.lower(Transcription.session_title).like(search_term)) | (func.lower(Transcription.cleaned_transcript_text).like(search_term)) |
            (func.lower(Transcription.topic_names_csv).like(search_term)) | (Transcription.session_detail.has(func.lower(SessionDetail.content).like(search_term)))
        )
    if purpose_filters:
...
def get_conflict_detail(conflict_id: int, db: Session = Depends(get_db)):
    conflict_db = db.query(Conflict).options(joinedload(Conflict.new_transcription_obj)).filter(Conflict.id == conflict_id).first()
    if not conflict_db: raise HTTPException(status_code=404, detail="Conflict not found")
    return ConflictDetailSchema(id=conflict_db.id, status=conflict_db.status, path_left=["KB Doc", conflict_db.existing_transcription_id], path_right=["Transcription", conflict_db.new_transcription_obj.session_title if conflict_db.new_transcription_obj else "N/A"], existing_content=conflict_db.existing_content_snippet, incoming_content=conflict_db.new_content_snippet, resolution_content=conflict_db.resolution_content)
