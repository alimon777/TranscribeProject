    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                url, index = futures[future]  # Retrieve the chunk and its index
                result = future.result()
                transcription.append((index, result))  # Store with index to maintain order

                # Delete the uploaded audio file from blob storage
                delete_response = requests.delete(url)
                if delete_response.status_code not in [200, 202, 204]:
                    print(
                        f"Failed to delete blob: Status code {delete_response.status_code}")
                else:
        ...
        sorted_transcriptions = ' '.join([transcription[1] for transcription in transcription])
...
async def transcribe_audio_async(processed_file_path, transcription_id, transcriptions, transcriptions_lock):
    try:
        ...
        with transcriptions_lock:
            ...
            transcriptions[transcription_id]["result"] = result
    except Exception as e:
        # If transcription fails, update the status to failed
        with transcriptions_lock:
            transcriptions[transcription_id]["status"] = "failed"
            transcriptions[transcription_id]["result"] = {"error": str(e)}